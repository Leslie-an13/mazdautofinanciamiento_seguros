    ...
    if($result_consult){
        ...
        if($result){
            $stmt_update = $pdo->prepare("UPDATE claims_approval_history 
                SET base_file_approved = ?, base_approval_status = ?, date_create_at = ? 
            ...
            $stmt_update->execute([
                'El usuario ' . $nombreCompleto . ' ha aprobado el archivo ' . date('Y-m-d H:i:s'),
                $select,
                date('Y-m-d H:i:s'),
                $year,
                $month
            ...
            if($rowsAffected > 0){
                echo json_encode([
                    'success' => true,
                    'message' => 'Se aprobo la base correctamente.'
                ]);
